...
// Match the bundle identifier domain (com.crivos.app) by default. Override in
// production via EMAIL_FROM env so the sender domain matches verified DNS records.
const FROM = process.env.EMAIL_FROM ?? 'CricOS <no-reply@crivos.app>';
...
//
// Every value that can come from user input (currently `name`) is HTML-escaped
// before being interpolated into the html body. Most webmail clients sanitize
// rendered email anyway, but desktop clients (Outlook, Thunderbird, Mail.app)
// will render raw HTML, and forwarded emails can land in systems that re-render
// our content. Cheap to escape; expensive if we don't.
